...
def laplacian_to_image(lpyr, filter_vec, coeff):
    ...
    #fig = plt.figure("TEST") #TODO remove
    for lvl in range(n, 0, -1):
        rec_im = expand(rec_im, filter_vec) + lpyr[lvl - 1]
        #fig.add_subplot(1, len(lpyr) - 1, n-lvl + 1) #TODO remove
        #plt.imshow(rec_im, cmap=plt.cm.gray) #TODO remove
    return rec_im
...
def render_pyramid(pyr, levels):
    ...
    for lvl in range(1, levels , 1):
        im_to_append = np.pad(image_stretch(pyr[lvl]), ((0, res.shape[0] - pyr[lvl].shape[0]), (0, 0)), 'constant', constant_values=0)
...
def pyramid_blending(im1, im2, mask, max_levels, filter_size_im, filter_size_mask):
    ...
    g_pyr_mask = build_gaussian_pyramid(mask.astype(np.float32), max_levels, filter_size_mask)[0]
...
def blending_example1():
    ...
    :return: The blended image.
    """
    im1 = read_image(relpath('external/soup_1024_1024.jpg'), REPRESENTATION_RGB) #TODO change folder
    im2 = read_image(relpath('external/waterpollo_1024_1024.jpg'), REPRESENTATION_RGB) #TODO change folder
    mask = read_image(relpath('external/waterpollo_mask_1024_1024.jpg'), REPRESENTATION_GRAYSCALE) #TODO change folder
    mask[mask > 0.5] = 1
    mask[mask <= 0.5] = 0
...
def blending_example2():
    ...
    :return: The blended image.
    """
    im1 = read_image(relpath('external/gal_1024_1024.jpg'), REPRESENTATION_RGB) #TODO change folder
    im2 = read_image(relpath('external/spider_1024_1024.jpg'), REPRESENTATION_RGB) #TODO change folder
    mask = read_image(relpath('external/spider_mask_1024_1024.jpg'), REPRESENTATION_GRAYSCALE) #TODO change folder
    mask[mask > 0.5] = 1
    mask[mask <= 0.5] = 0
    mask = mask.astype(np.bool)
