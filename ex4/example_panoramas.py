...
#
#   # Read images.
#   ims = [sol4_utils_art.read_image(f,1) for f in files]
#   # Extract feature point locations and descriptors.
#   def im_to_points(im):
#     pyr,_ = sol4_utils_art.build_gaussian_pyramid(im, 3, 7)
#     return sol4_art.find_features(pyr)
#   p_d = [im_to_points(im) for im in ims]
#
#   # Compute homographies between successive pairs of images.
#   Hs = []
...
#
#     # Find matching feature points.
#     ind1, ind2 = sol4_art.match_features(desc1, desc2, .7)
#     points1, points2 = points1[ind1,:], points2[ind2,:]
#
#     # Compute homography using RANSAC.
#     H12, inliers = sol4_art.ransac_homography(points1, points2, 10000, 6)
#
#     # Display inlier and outlier matches.
#     sol4_art.display_matches(ims[i], ims[i+1], points1 , points2, inliers=inliers)
#     Hs.append(H12)
#
#   # Compute composite homographies from the panorama coordinate system.
#   Htot = sol4_art.accumulate_homographies(Hs, (num_images-1)//2)
#
#   # Final panorama is generated using 3 channels of the RGB images
#   ims_rgb = [sol4_utils_art.read_image(f,2) for f in files]
#
#   # Render panorama for each color channel and combine them.
#   panorama = [sol4_art.render_panorama([im[...,i] for im in ims_rgb], Htot) for i in range(3)]
#   panorama = np.dstack(panorama)
#
#   #plot the panorama
#   plt.figure(figsize=figsize)
